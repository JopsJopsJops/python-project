...
class DashboardWidget(QWidget):
    ...
    def init_summary_tab(self):
        ...
        right_layout.addWidget(budget_scroll)  
    ...
    def safe_update_dashboard(self):
        ...
        if hasattr(self, "update_summary_tab")\
              and self.summary_table is not None:
            self.update_summary_tab()
    ...
    def safe_update_summary_tab(self):
        ...
        if hasattr(subtotals, "__class__")\
              and subtotals.__class__.__name__ == "Mock":
            return
    ...
    def generate_insights(self, category_totals, total_all):
        """Generate meaningful and \
            actionable insights from spending data - ENHANCED"""
        ...
        try:
            ...
            if sorted_categories:
                ...
                insights.append(
                    f"📈 <b>Spends:</b> \
                        ₱{top_amount:,.0f} ({top_percentage:.1f}%)"
                )
            ...
            if len(sorted_categories) >= 3:
                ...
                insights.append(
                    f"🎯 <b>Top 3 Categories:</b> \
                        {top3_percentage:.1f}% of total"
                )
            ...
            if len(monthly_totals) >= 2:
                ...
                insights.append\
                    (f"📅 <b>Monthly Average:</b> ₱{avg_monthly:,.0f}")

                # Trend analysis
                if len(recent_months) >= 2:
                    trend = (
                        ((latest_month - \
                          recent_months[-2]) / recent_months[-2]) * 100
            ...
            if diversity_score >= 6:
                ...
                recommendations.append\
                    ("Try categorizing expenses more specifically")

            # Budget alerts integration
            if hasattr(self.data_manager, "budget_manager"):
                budget_alerts = \
                    self.data_manager.budget_manager.check_budget_alerts()
                if budget_alerts:
                    budget_warnings = \
                        [alert for alert in budget_alerts if "🚨" in alert]
                    if budget_warnings:
                        warnings.append\
                            (f"{len(budget_warnings)} budget(s) exceeded")
        ...
        # Add data summary
        insights_text += (
            f"<br><small><i>Based on \
                {len(sorted_categories)} categories</i></small>"
        )
    ...
    def update_budget_alerts(self):
        """Update budget alerts display \
            - FIXED: Handle no budgets case properly"""
        try:
            if (
            ...
            if hasattr(self.data_manager, "budget_manager"):
                ...
                logger.info(
                    f"📢 Found {len(alerts)} budget alerts, \
    budgets exist: {budgets_exist}"
                )

                if not budgets_exist:
                    # No budgets set - \
                    # show friendly info message (not red alert)
                    self.budget_alerts_label.setText(
                        "💡 No monthly budgets set yet.\n\n"
                        "Use Budget Management to set spending limits and \
    track your expenses."
                    )
                    self.budget_alerts_label.setStyleSheet(
                        """
            ...
            if hasattr(self, "budget_alerts_label") \
                and self.budget_alerts_label:
                self.budget_alerts_label.setText\
                    ("❌ Error loading budget alerts")
                self.budget_alerts_label.setStyleSheet(
                    """
                    QLabel {
                        background-color: #442222;
    ...
    def update_charts_tab(self):
        ...
        self.pie_ax.clear()
        if amounts:
            top_categories, top_amounts = \
                prepare_chart_data(categories, amounts)
            explode = [
                0.05 if a < (sum(top_amounts) * 0.01) \
        ...
        if amounts:
            ...
            if sorted_data:
                ...
                self.bar_ax.grid(True, alpha=0.2, \
    ...
    def on_pie_click(self, event):
        ...
        if category == "Others":
            ...
            # Get the top categories that are shown\
            #  individually (not in "Others")
            categories_list, amounts_list = \
                aggregate_category_totals(filtered_data)
            top_categories, top_amounts = prepare_chart_data(
                categories_list, amounts_list
            )

            # Find which categories are in\
            #  "Others" (all categories minus top categories)
            other_categories = [
            ...
            total_amount = \
                sum(float(exp.get("amount", 0)) for exp in other_expenses)
            ...
            total_amount = \
                sum(float(exp.get("amount", 0)) for exp in category_expenses)

            print(
                f"DEBUG: Regular category - Amount: {total_amount}, \
    Expenses: {len(category_expenses)}"
            )
    ...
    def on_bar_click(self, event):
        ...
        try:
            ...
            if closest_index < len(tick_labels):
                ...
                print(f"DEBUG: Selected category: \
    ...
    def show_other_categories_details(self, categories, total_amount, expenses):
        ...
        header_label = \
            QLabel(f"💎 Other Categories - Total: ₱{total_amount:,.2f}")
        header_label.setStyleSheet(
            """
            QLabel {
                color: #00ffff;
        ...
        if expenses:
            ...
            if len(expenses) > 1:
                ...
                summary_label = QLabel(
                    f"📈 Average per expense:\
                ₱{avg_amount:,.2f} ({len(expenses)} transactions)"
                )
    ...
    def update_trends_tab(self):
        months, totals = \
            prepare_trend_data(self.data_manager.get_monthly_totals())
        ...
        if months:
            ...
            self.trend_ax.set_ylabel(
                "Total Expenses (₱)", \
                    color="#e0e0e0", fontweight="bold", fontsize=12
            )
 ...
 self.is_dark_color(
    ...
    def delete_expense(self, category, record):
        ...
        <h3>🗑️ Are you sure you want\
              to delete this expense?</h3>
        <p><b>📊 Category:</b> {category}</p>
        <p><b>💰 Amount:</b> ₱{amount:,.2f}</p>
        <p><b>📅 Date:</b> {date}</p>
        <p><b>📝 Description:</b> {description}\
            {'...' if len(description) >= 50 else ''}</p>
        <br>
        <p style="color: #ffb86c;"><b>💡\
          You can use the Undo button if you change your mind</b></p>
        """
        ...
        if self.data_manager.delete_expense(category, record):
            ...
            QMessageBox.information(
                ...
                "<p><font color='#ffb86c'><b>💡\
              Tip: Use the '↶ Undo' button \
                to restore if needed.</b></font></p>",
                QMessageBox.Ok,
            )
    ...
    def search_expenses(self, keyword=None):
        ...
        logger.debug(
            "Search executed for keyword='%s', \
                %d results found", keyword, len(results)
        )
    ...
    def show_expense(self):
        ...
        if hasattr(expenses, "__class__") and \
            expenses.__class__.__name__ == "Mock":
    ...
    def render_table(self, data, show_totals=False, is_search=False):
        ...
        def add_action_row(row_data, record):
            ...
            self.table.setItem\
                (row, 3, QTableWidgetItem(formatted["description"]))
    ...
    def exit_mode(self):
        ...
        if reply == QMessageBox.Yes:
            ...
            __import__("PyQt5.QtWidgets", \
                       fromlist=["QApplication"]).QApplication.quit()
...
class BudgetDialog(QDialog):
    ...
    def setup_set_budget_tab(self):
        ...
        self.amount_input.setSizePolicy\
            (QSizePolicy.Expanding, QSizePolicy.Fixed)
    ...
    def get_scrollbar_stylesheet(self):
        """Return minimal scrollbar styling \
            (global styles handle most of it)"""
        return """
            QScrollArea {
                border: 1px solid #404040;
                border-radius: 6px;
    ...
    def update_budgets_table(self):
        ...
        if hasattr(self.data_manager.budget_manager, "budgets"):
            ...
            self.budgets_table.setColumnWidth(3, 85)  # Actions
    ...
    def update_alerts(self):
        ...
        if not budgets_exist:
            ...
            self.alerts_text.setHtml(
                ...
                "Great job! All your spending is within \
                    the budget limits you've set."
            )
            self.alerts_text.setStyleSheet(
                """
                QTextEdit {
    ...
    def update_progress_bars(self):
        ...
        if not hasattr(self, "progress_container") or \
            self.progress_container is None:
        ...
        if not budgets:
            ...
            for category, budget in budgets.items():
                ...
                percentage = \
                    min((spending / budget) * 100, 100) if budget > 0 else 0

                print(
                    f"DEBUG: Creating bar chart for\
    ...
    def create_bar_chart_widget(self, category, spending, budget, percentage):
        ...
        status_text = (
            "OVER" if percentage > 100 else \
                "WARNING" if percentage > 80 else "GOOD"
        )
