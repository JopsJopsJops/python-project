    ...
    def import_from_csv(file_path, data_manager=None):
        ...
        try:
            if not isinstance(file_path, (str, bytes, os.PathLike)):
                raise TypeError(
                    f"Expected file path, got {type(file_path).__name__}")

            with open(file_path, newline="", encoding="utf-8") as csvfile:
                reader = csv.DictReader(csvfile)
                if "category" not in reader.fieldnames:
                    logger.warning(
                        "Invalid CSV format: missing category column")
                    return {}
    ...
    def import_from_excel(file_path, data_manager=None):
        ...
        try:
            ...
            headers = [str(c.value).strip().lower()
                       if c.value else "" for c in sheet[1]]
            if "category" not in headers:
                logger.warning("Invalid Excel format: missing category column")
                return {}

            category_idx = headers.index("category")
            amount_idx = headers.index(
                "amount") if "amount" in headers else None
            date_idx = headers.index("date") if "date" in headers else None
            desc_idx = headers.index(
                "description") if "description" in headers else None

            for row in sheet.iter_rows(min_row=2, values_only=True):
                if not any(row):
                    continue
                try:
                    raw_category = row[category_idx] if category_idx < len(
                        row) else None
                    category = str(raw_category).strip(
                    ) if raw_category else "Uncategorized"
                    category = category.lower()   # ✅ normalize

                    amount_val = row[amount_idx] if (
                        amount_idx is not None and amount_idx < len(row)) else 0
                    try:
                    ...
                    date = str(row[date_idx]) if (
                        date_idx is not None and date_idx < len(row) and row[date_idx]) else ""
                    description = str(row[desc_idx]) if (
                        desc_idx is not None and desc_idx < len(row) and row[desc_idx]) else ""
