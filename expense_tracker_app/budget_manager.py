...
class BudgetManager:
    ...
    def load_budgets(self):
        ...
        try:
            with open(self.budget_file, "r", encoding="utf-8") as f:
                ...
                logger.info(
                    f"💰 Loaded {len(self.budgets)} {budget_period} budgets (last updated: {last_updated})"
                )
    ...
    def _get_monthly_spending(self, category: str, month: str) -> float:
        ...
        try:
            ...
            logger.debug(
                f"🔍 Checking {len(all_expenses)} total expenses for category: {category}, month: {month}"
            )
            ...
            for expense in all_expenses:
                ...
                if (
                    ...
                    try:
                        ...
                        logger.debug(
                            f"   ➕ Added expense: {expense_category} - ₱{expense_amount} on {expense_date}"
                        )
